...
The short answer is that Qwik solves a problem that other frameworks can't solve. Qwik has instant-on startup performance no matter the application complexity. Ie, Qwik app deliver the same amount of initial JS regarless of the amount of components. [Qwik is the first open-source O(1) framework](https://www.builder.io/blog/our-current-frameworks-are-on-we-need-o1).
...
We designed Qwik to be [extremelly easy to learn](../cheat/qwik-react/index.mdx) and become productive for React developers, component authoring is pretty much the same, and routing is inspired by Nextjs and others.
...
We also got an interactive [tutorial](../../tutorial/welcome/overview/) to get you started.

### What are all those `$`?

You might have noticed that there are more [`$`](../advanced/optimizer/index.mdx#the-meaning-of-) than usual in Qwik apps: `component$()`, `useWatch$()`, `<div onClick$={() => console.log('click')} />`. It's not for nothing, Qwik breaks your application into small chunks, this pieces are smaller than the component itself, event handlers, hooks... the `$` signals both the [optimizer](../advanced/optimizer/index.mdx) and the developer when it's happening.
...
The component above is splitted thanks to the `$` syntax into:
...
You can deploy easily a Qwik App in any [serverless enviroment thanks to our adaptors](../../qwikcity/middleware/overview/index.mdx), we also support a [vanilla-node adaptor](../../qwikcity/middleware/express/index.mdx) for Express-based servers.
...
MPA vs SPA is not longer an architectural decision taken at beginning of the project, but a decision made for every link.
...
No quite, other frameworks suggest removing all the `<Scripts>` at the root to generate an MPA, efectively removing all the interactivity along with the SPA navigation.

And if `<Scripts>` are not removed, then full page reloads become very expensive, because every page reloads means that the framework need to hydrate the entire page.

Qwik does not have any of this page.
...
YES, there is a growing community of Qwik developers at [Discord](https://discord.gg/bNVSQmPzqy) and [Github](https://github.com/BuilderIO/qwik), they are making amazing contributions to the framework, building sites at scale and helping each other. [Join us](https://discord.gg/bNVSQmPzqy).


### Is Qwik production ready?

YES! but it depends. Qwik has been in development for 2 years now, and we reached [the beta milestones](https://www.builder.io/blog/qwik-and-qwik-city-have-reached-beta), meaning that we are confident that Qwik is ready for production, and there is not expected breaking changes.
